        ...
        <el-form label-width="120px">
          ...
          <el-row>
            <el-form-item label="服务名称：">
              <el-input v-model="searchObj.skuName" clearable @keyup.enter.native="getData(1)"></el-input>
            </el-form-item>
          </el-row>
        </el-form>
        <div class="search-button-container">
          <el-button @click="getData(1)" type="primary" size="small" >查询关联服务</el-button>
        </div>
      </search-field>
...
export default {
  ...
  data () {
    return {
      dialogVisible: false,
      searchObj: this.lk.deepCopy(models.supplyProduct),
      skus: [],
      options: {
        data: [],
        columns: [{
  ...
  methods: {
    ...
    ThreeChange (result) {
      if (result.one) {
        this.searchObj.categoryId = result.one
      }
      if (result.two) {
        this.searchObj.categoryId = result.one
      }
      if (result.three) {
        this.searchObj.categoryId = result.three
      }
    },
    // 获取列表
    getData (page) {
      let url = '/api/product/query/jms/tom_merchant_product'
        ...
        .then(result => {
          if (result.data.statusCode === 200) {
            this.options.data = result.data.data.list.map(item => {
              item.id = item.skuId
              item.price = item.coefficient ? item.coefficient : item.settlePrice
